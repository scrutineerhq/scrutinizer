 ...
 */
add_action( 'init', 'scrutinizer_test_init', 10 );
function scrutinizer_test_init() {
 ...
 */
add_action( 'wp_loaded', 'scrutinizer_test_http_calls', 10 );
function scrutinizer_test_http_calls() {
 ...
 */
add_action( 'template_redirect', 'scrutinizer_test_options', 10 );
function scrutinizer_test_options() {
 ...
 */
add_action( 'wp_enqueue_scripts', 'scrutinizer_test_enqueue', 10 );
function scrutinizer_test_enqueue() {
	wp_register_script( 'sct-test-demo', false, array(), '1.0.0', true );
	wp_enqueue_script( 'sct-test-demo' );
	wp_add_inline_script( 'sct-test-demo', '/* Scrutinizer Test Plugin — demo inline script */' );

	wp_register_style( 'sct-test-demo-style', false, array(), '1.0.0' );
	wp_enqueue_style( 'sct-test-demo-style' );
	wp_add_inline_style( 'sct-test-demo-style', '/* Scrutinizer Test Plugin — demo inline style */' );
}

/**
 * Add a filter on the_content to simulate content filtering.
 */
add_filter( 'the_content', 'scrutinizer_test_content_filter', 10 );
function scrutinizer_test_content_filter( $content ) {
